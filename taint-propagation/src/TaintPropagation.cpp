...
/// The maximum number of instructions to track per lattice value. Once the
/// number exceeds this threshold, the lattice value becomes overdefined.
...
/// A DefInst can reach a UseInst only if the UseInst is dominated by the
/// DefInst or one of its iterated dominance frontiers NOTE:
/// - Specail case: if DefInsts is empty, we consider DefInsts can reach UseInst
/// - UseInst should not occur in DefInsts
static bool reachable(const std::vector<Instruction *> &DefInsts,
                      Instruction *UseInst) {
  if (DefInsts.empty())
    return true;
...
/// groups. The register group represents SSA registers, the memory group
/// represents in-memory values, and the return group represents the return
/// values of functions. An LLVM Value can technically be in more than one
/// group.
...
public:
  ...
  enum TaintLatticeStateTy { Undefined, Tainted, Overdefined, Untracked };
  ...
  TaintLatticeVal(TaintLatticeStateTy LatticeState)
      : LatticeState(LatticeState) {}
  TaintLatticeVal(std::vector<Instruction *> &&TaintedAtInsts)
      : LatticeState(Tainted), TaintedAtInsts(std::move(TaintedAtInsts)) {
    assert(std::is_sorted(this->TaintedAtInsts.begin(),
                          this->TaintedAtInsts.end(), Compare()));
  }
  ...
  /// corresponding lattice key refers to arguments or return values of a
  /// function.
  ...
  bool isTainted() const { return LatticeState == Tainted; }

  /// Just set the lattic value to Tainted state, there is no change to the
  /// instructions set `TaintedAtInsts`
  void setTainted() { LatticeState = Tainted; }

  bool operator==(const TaintLatticeVal &RHS) const {
    return LatticeState == RHS.LatticeState &&
           TaintedAtInsts == RHS.TaintedAtInsts;
  }
...
/// It also computes the lattice values that change as a result of executing
/// instructions.
...
public:
  ...
  TaintLatticeVal getUndefVal() const { return TaintLatticeVal::Undefined; }

  TaintLatticeVal getOverdefinedVal() const {
    return TaintLatticeVal::Overdefined;
  }

  TaintLatticeVal getUntrackedVal() const { return TaintLatticeVal::Untracked; }

  bool IsUntrackedValue(TaintLatticeKey Key);

  /// ComputeLatticeVal - Compute and return a TaintLatticeVal corresponding to
  /// the given TaintLatticeKey.
  ...
  /// these cases, we simply union the instruction sets. If the size of the
  /// union is greater than the maximum instructions we track, the merged value
  /// is overdefined.
...
private:
  /// Handle PHINode. The PHINode state is the merge of the incoming values
  /// states
  ...
  void
  visitGetElementPtr(GetElementPtrInst &I,
                     DenseMap<TaintLatticeKey, TaintLatticeVal> &ChangedValues,
                     TaintSolver &TS);

  /// Handle LoadInst. The loaded value state is the merge of the loaded value
  /// state with the pointer operand state.
  void visitLoad(LoadInst &I,
                 DenseMap<TaintLatticeKey, TaintLatticeVal> &ChangedValues,
                 TaintSolver &TS);

  /// Handle StoreInst. If the stored value is Tainted, we set the state of the
  /// pointer operand to Tainted and track the values that the pointer operand
  /// depends on. We also set the state of the values that the pointer operand
  /// depends on to Tainted and handle the merge of pointer type arguments in
  /// this function.
  ...
  void
  visitMemTransfer(MemTransferInst &I,
                   DenseMap<TaintLatticeKey, TaintLatticeVal> &ChangedValues,
                   TaintSolver &TS);
...
class TaintSolver {
  ...
  /// ValueDependencyMap - Map a value to a set of values that the value depends
  /// on.
  const std::unordered_map<Value *, SmallPtrSet<Value *, 16>>
      &ValueDependencyMap;
...
public:
  ...
  /// getExistingValueState - Return the TaintLatticeVal object corresponding to
  /// the given value from the ValueState map. If the value is not in the map,
  /// UntrackedVal is returned, unlike the getValueState method.
  TaintLatticeVal getExistingValueState(TaintLatticeKey Key) const;

  /// getValueState - Return the TaintLatticeVal object corresponding to the
  ...
  /// getDependency - Return a set of values that the given value depends on
  /// according to ValueDependencyMap.
...
private:
  /// UpdateState - When the state of some TaintLatticeKey is potentially
  /// updated to the given TaintLatticeVal, this function notices and adds the
  /// LLVM value corresponding the key to the work list, if needed.
...
bool TaintLatticeFunc::IsUntrackedValue(TaintLatticeKey Key) { return false; }

TaintLatticeVal TaintLatticeFunc::ComputeLatticeVal(TaintLatticeKey Key) {
  return getUndefVal();
}

bool TaintLatticeFunc::IsSpecialCasedPHI(PHINode *PN) { return true; }

TaintLatticeVal TaintLatticeFunc::MergeValues(TaintLatticeVal X,
                                              TaintLatticeVal Y) {
...
void TaintLatticeFunc::ComputeInstructionState(
  ...
  case Instruction::Call: {
    if (auto *MTI = dyn_cast<MemTransferInst>(&I)) {
      ...
      return visitCallSite(cast<CallBase>(&I), ChangedValues, TS);
    }
  }
...
void TaintLatticeFunc::visitStore(
  ...
  // If the type of some function arguments is pointer, inform the solver that
  // the caller function is executable, and perform merges for these pointer
  // arguments. NOTE: This part of code *must* be placed in `visitStore`
  // FIXME: Is there any way to avoid perform merges for function pointer
  // arguments when the pointer operand of StoreInst and the set of values that
  // the pointer operand depends are unrelated to function pointer arguments?
  ...
  for (Argument *Arg : AffectedFnPointerArguments) {
    for (User *U : F->users()) {
      if (auto *CS = dyn_cast<CallBase>(U)) {
        TS.MarkBlockExecutable(CS->getParent());
        auto ArgActual = TaintLatticeKey(CS->getArgOperand(Arg->getArgNo()),
        ...
        if (TS.getValueState(ArgFormal).isTainted()) {
          ChangedValues[ArgActual] =
              MergeValues(TS.getValueState(ArgActual), TaintLatticeVal({CS}));
        }
      }
    }
  }
...
void TaintLatticeFunc::visitCallSite(
    CallBase *CS, DenseMap<TaintLatticeKey, TaintLatticeVal> &ChangedValues,
    TaintSolver &TS) {
  ...
  do {                                                                         \
    if (F && F->getName().equals(FUNC_NAME)) {                                 \
      ...
      for (const auto &i : Args) {                                             \
        if (i == -1) {                                                         \
          auto Reg = TaintLatticeKey(CS, IPOGrouping::Register);               \
          ChangedValues[Reg] =                                                 \
              MergeValues(TS.getValueState(Reg), TaintLatticeVal({CS}));       \
          updateDependencyValueState(CS, TaintLatticeVal({CS}), ChangedValues, \
                                     TS);                                      \
        } else {                                                               \
          auto Reg =                                                           \
              TaintLatticeKey(CS->getOperand(i), IPOGrouping::Register);       \
          ChangedValues[Reg] =                                                 \
              MergeValues(TS.getValueState(Reg), TaintLatticeVal({CS}));       \
          updateDependencyValueState(CS->getOperand(i), TaintLatticeVal({CS}), \
                                     ChangedValues, TS);                       \
        }                                                                      \
      }                                                                        \
      return;                                                                  \
  ...
  do {                                                                         \
    if (F && F->getName().equals(FUNC_NAME)) {                                 \
      ...
      for (const auto &i : SrcArgs) {                                          \
        auto Reg = TaintLatticeKey(CS->getOperand(i), IPOGrouping::Register);  \
        if (TS.getValueState(Reg).isTainted() &&                               \
            reachable(TS.getValueState(Reg).getTaintedAtInsts(), CS)) {        \
          SrcTainted = true;                                                   \
          break;                                                               \
        }                                                                      \
      }                                                                        \
      if (SrcTainted) {                                                        \
        for (const auto &i : DstArgs) {                                        \
          ...
          {                                                                    \
            ChangedValues[Reg] =                                               \
                MergeValues(TS.getValueState(Reg), TaintLatticeVal({CS}));     \
            updateDependencyValueState(                                        \
                CS->getOperand(i), TaintLatticeVal({CS}), ChangedValues, TS);  \
          }                                                                    \
        }                                                                      \
      }                                                                        \
    }                                                                          \
...
#undef HANDLE_TAINT_PROPAGATION_LIBCALL
  ...
  // If this is an indirect call or we can't track the function, there's nothing
  // to do.
  ...
  for (Argument &Arg : F->args()) {
    ...
    if (TS.getValueState(ArgActual).isTainted() &&
        reachable(TS.getValueState(ArgActual).getTaintedAtInsts(), CS)) {
  ...
  auto RegI = TaintLatticeKey(CS, IPOGrouping::Register);
  if (TS.getValueState(RetF).isTainted()) {
    ChangedValues[RegI] =
        MergeValues(TS.getValueState(RegI), TaintLatticeVal({CS}));
  }
}
...
void TaintLatticeFunc::visitCast(
  ...
  if (TS.getValueState(Src).isTainted() &&
    ...
    ChangedValues[Dst] =
        MergeValues(TS.getValueState(Dst), TaintLatticeVal({&I}));
  }
}
...
void TaintSolver::UpdateState(TaintLatticeKey Key, TaintLatticeVal LV) {
  ...
  // Update the state of the given TaintLatticeKey and add its corresponding
  // LLVM value to the work list.
...
#if LLVM_VERSION_MAJOR >= 8
void TaintSolver::getFeasibleSuccessors(Instruction &TI,
...
#undef HANDLE_TAINT_SOURCE
  ...
  // Solver our custom lattice. In doing so, we will also get tainted
  // instructions
  Solver.Solve();
...
static RegisterPass<TaintPropagationLegacyPass>
    X(DEBUG_TYPE, "Taint Propagation",
